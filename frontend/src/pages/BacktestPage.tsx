/**
 * BacktestPage — Gerçek Simülasyon
 *
 * Kullanıcı bir anket + tarih seçer.
 * Backend, o tarih itibarıyla factor scoring yaparak portföy oluşturur (lookahead-free).
 * Oluşturulan portföyün gerçek H2 performansı yfinance verisiyle hesaplanır.
 */
...
import { poolService, assessmentService } from '@/services'
...
function calcMaxDrawdown(series: number[]): number {
  let peak = 0, maxDD = 0
  series.forEach(v => { if (v > peak) peak = v; const dd = peak - v; if (dd > maxDD) maxDD = dd })
  return maxDD
}
...
async function fetchHoldings(
  ...
  for (const inst of instruments) {
    try {
      ...
      holdings.push({
        ticker: inst.ticker, name: inst.name, assetClass: inst.assetClass, weight: inst.weight,
        priceH1Start: h1[0].close, priceH1End: h1[h1.length - 1].close, priceH2End: h2[h2.length - 1].close,
        h1Return, h2Return, contribution: inst.weight * h2Return, monthlyH2: monthlyH2Returns(h2),
      })
    } catch {
...
function StatCard({ label, value, sub, positive }: { label: string; value: string; sub?: string; positive?: boolean | null }) {
  const clr = positive == null ? 'text-stone-900 dark:text-stone-100' : positive ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'
  return (
...
export default function BacktestPage() {
  ...
  const [simAsOfDate,        setSimAsOfDate        ] = useState('')
  ...
  const runSimulation = useCallback(async () => {
    ...
    const backtestYear = Math.max(2022, Math.min(currentYear - 1, asOfMonth <= 6 ? asOfYear : asOfYear + 1))
    ...
    try {
      const answersData  = await assessmentService.getAnswers(selectedAssessment.assessmentId)
      ...
      const holdings = await fetchHoldings(instruments, backtestYear, language, msg => setSoftErr(msg))

      if (holdings.length === 0) {
        setFatalErr(language === 'tr' ? 'Hiçbir varlık için fiyat verisi alınamadı.' : 'No price data available for any asset.')
        setLoading(false); return
      }
      ...
      setResult({
        ...
        year:             backtestYear,
        holdings,
        totalH2:          holdings.reduce((s, h) => s + h.contribution, 0),
        totalH1:          holdings.reduce((s, h) => s + h.weight * h.h1Return, 0),
  ...
  const profileLabel = result
    ? (PROFILE_LABELS[result.portfolio.profileType] ?? result.portfolio.profileType)
    : ''
          ...
          <div>
            ...
            <p className="text-sm text-stone-600 dark:text-stone-300 leading-relaxed">
              {language === 'tr'
                ...
                : 'Select an assessment and date. The system builds a portfolio using only market data up to that date — no lookahead. Then real H2 performance is calculated.'}
            </p>
          </div>
            ...
            <div className="flex-1 min-w-[180px]">
              ...
              <p className="text-xs text-stone-400 dark:text-stone-500 mt-1">
                {language === 'tr'
                  ...
                  : 'Portfolio is built as of this date. H2 performance calculated automatically.'}
              </p>
            </div>
        ...
        {result && !loading && (
          <>
            ...
            <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50/50 dark:bg-violet-950/20 px-4 py-3 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-violet-600 dark:text-violet-400 mt-0.5">
                  {language === 'tr'
                    ? `Portföy ${result.simulatedAsOf} tarihi itibarıyla oluşturuldu. ${result.year} H2 (Tem–Ara) performansı gerçek fiyatlarla hesaplandı.`
                    : `Portfolio built as of ${result.simulatedAsOf}. ${result.year} H2 (Jul–Dec) performance calculated from real prices.`}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard label={`${t('backtest.h2Return')} (${result.year})`} value={fmtPct(result.totalH2)} sub={t('backtest.h2Sub')} positive={result.totalH2 >= 0} />
              <StatCard label={`${t('backtest.h1Signal')} (${result.year})`} value={fmtPct(result.totalH1)} sub={t('backtest.h1Sub')} positive={result.totalH1 >= 0} />
              <StatCard label={t('backtest.winners')} value={`${Math.round(result.winRate * 100)}%`} sub={`${result.holdings.filter(h => h.h2Return > 0).length} / ${result.holdings.length} ${t('backtest.assets')}`} positive={result.winRate >= 0.5} />
              <StatCard label={t('backtest.sharpe')} value={result.sharpe.toFixed(2)} sub={`${t('backtest.maxDrawdown')} ${fmtPct(-result.maxDrawdown)}`} positive={result.sharpe >= 0.5} />
            </div>
              ...
              <div className="card">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <LineChart data={cumulChartData}>
                    ...
                    <Line type="monotone" dataKey="getiri" name={language === 'tr' ? 'Portföy' : 'Portfolio'} stroke={result.totalH2 >= 0 ? '#639922' : '#E24B4A'} strokeWidth={2} dot={{ r: 4 }} activeDot={{ r: 6 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
              ...
              <div className="card">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={barData} barSize={12} barGap={2}>
                    ...
                    <Tooltip contentStyle={tipStyle} formatter={(v: number, name: string) => [`${v >= 0 ? '+' : ''}${v.toFixed(2)}%`, name === 'h1' ? t('backtest.h1Period') : t('backtest.h2ReturnCol')]} />
                    ...
                    <Bar dataKey="h2" name="h2" radius={[3, 3, 0, 0]}>
                      {barData.map((entry, i) => <Cell key={i} fill={entry.h2 >= 0 ? '#639922' : '#E24B4A'} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
                <div className="flex gap-4 mt-3 text-xs text-stone-400 dark:text-stone-500">
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm" style={{ background: isDark ? '#44403c' : '#d6d3d1' }} />{t('backtest.h1Period')}</span>
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-green-600" />{t('backtest.h2Positive')}</span>
                  <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-red-500" />{t('backtest.h2Negative')}</span>
                </div>
              </div>
            </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
                    {[...result.holdings].sort((a, b) => b.h2Return - a.h2Return).map(h => {
                      ...
                      return (
                        <tr key={h.ticker} className="hover:bg-stone-50/50 dark:hover:bg-stone-800/30 transition-colors">
                          <td className="py-3 pr-4">
                            ...
                            {h.name !== h.ticker && <p className="text-xs text-stone-400 dark:text-stone-500 truncate max-w-[140px]">{h.name}</p>}
                          </td>
                          <td className="py-3 pr-4">
                          ...
                          <td className="py-3 pr-4 text-right text-stone-700 dark:text-stone-300 font-medium text-xs">{(h.weight * 100).toFixed(1)}%</td>
                          <td className="py-3 pr-4 text-right text-stone-400 dark:text-stone-500 font-mono text-xs">{h.priceH1Start.toFixed(2)}</td>
                          <td className={`py-3 pr-4 text-right text-xs font-medium ${h.h1Return >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{fmtPct(h.h1Return)}</td>
                          <td className={`py-3 pr-4 text-right font-medium ${h.h2Return >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{fmtPct(h.h2Return)}</td>
                          <td className={`py-3 text-right text-xs ${h.contribution >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{fmtPct(h.contribution)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
                  <tfoot>
                    <tr className="border-t-2 border-stone-200 dark:border-stone-600">
                      <td colSpan={6} className="pt-3 text-sm font-medium text-stone-700 dark:text-stone-300">{t('backtest.totalH2')}</td>
                      <td className={`pt-3 text-right font-bold text-base ${result.totalH2 >= 0 ? 'text-green-700 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{fmtPct(result.totalH2)}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            {/* Verdict */}
            <div className={`card flex gap-4 items-start ${result.totalH2 >= 0.08 ? 'border-green-200 dark:border-green-800 bg-green-50/30 dark:bg-green-950/20' : result.totalH2 >= 0 ? 'border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-950/20' : 'border-red-200 dark:border-red-800 bg-red-50/30 dark:bg-red-950/20'}`}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${result.totalH2 >= 0.08 ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400' : result.totalH2 >= 0 ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400' : 'bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400'}`}>
                {result.totalH2 >= 0.08 ? '✓' : result.totalH2 >= 0 ? '~' : '✕'}
              </div>
              <div>
                <p className={`text-sm font-medium mb-1.5 ${result.totalH2 >= 0.08 ? 'text-green-800 dark:text-green-300' : result.totalH2 >= 0 ? 'text-amber-800 dark:text-amber-300' : 'text-red-700 dark:text-red-400'}`}>
                  {t('backtest.validResult').replace('{profile}', profileLabel).replace('{year}', String(result.year))}
                </p>
